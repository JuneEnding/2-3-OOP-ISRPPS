...
namespace ClassLibraryModel
{
    public class Model
    {
        ...
        public int timer { get; set; } = 0;     // флаг таймера
        public int countel = 0;
        public bool selectrel = false;
        private int curentel = -1; // Текущий выделенный элемент
        ...
        public float PointInLine(Point p1, Point p2, Point p)
        {
            int e = 10;
            float t;
            if ((Math.Min(p1.X, p2.X) - e <= p.X && p.X <= Math.Max(p1.X, p2.X) + e) &&                       // во первых
                (Math.Min(p1.Y, p2.Y) - e <= p.Y && p.Y <= Math.Max(p1.Y, p2.Y) + e)
                )
            {
                if ((-e <= (p2.X - p1.X) && (p2.X - p1.X) <= e) ||                                          // во вторых
                    (-e <= (p2.Y - p1.Y) && (p2.Y - p1.Y) <= e))
                {
                    return 0;
                }
                t = (float)(p.Y - p1.Y) / (float)(p2.Y - p1.Y) - (float)(p.X - p1.X) / (float)(p2.X - p1.X);// в третих
                return t;
            }
