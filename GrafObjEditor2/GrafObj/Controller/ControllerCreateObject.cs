...
namespace GrafObj
{
    public class ControllerCreateObject: Controller.ControllerDiagram
    {
        ...
        public class Diagramm
        {
                ...
                for (int i = 0 ; i < maxi; i++)
                    for (int j = 0; j < maxi; j++)
                    {
                        ...
                        if (model.adjmatrix[i * 10 + jj].direction > 0)
                        {
                            ...
                            AddRelTo(elj.name, eli.name, Color.FromName(eli.color), model.adjmatrix[i * 10 + jj].direction);
                            AddRelFrom(eli.name, elj.name, Color.FromName(eli.color), 0);
                        }
                    }
            }
            ...
            public void AddObj(string name, string color)
            {
                objects.Add(new Obj(name, Color.FromName(color)));
            }
